...
async function getPopularSeries(): Promise<Movie[]> {
  const response = await fetch('http://localhost:3000/api/tmdb/tv/popular', {
    cache: 'no-cache',
  })
  return response.json()
}
...
async function getTopRatedMovies(): Promise<Movie[]> {
  const response = await fetch('http://localhost:3000/api/tmdb/movie/top_rated', {
    cache: 'no-cache',
  })
  return response.json()
}
...
async function getTopRatedSeries(): Promise<Movie[]> {
  const response = await fetch('http://localhost:3000/api/tmdb/tv/top_rated', {
    cache: 'no-cache',
  })
  return response.json()
}
...
async function getHeroContent(): Promise<Movie & { type: 'tv' | 'movie' }> {
  ...
  const response = await fetch(`http://localhost:3000/api/tmdb/${type}/popular`, {
    cache: 'no-cache',
  })
